...
The top-level parser function is defined as follows:
\begin{minted}{haskell}
...
\code{varDeclP} parser, followed by a non-empty sequence of function
declarations using the \code{funDeclP} parser.
...
error if the resulting list of \haskell{FunDecl}s is empty, informing the user
that a program must define a \spl{main()} function.
...
between braces `\verb|{...}|' using the \code{parseToNextStmt} function for
error recovery.
