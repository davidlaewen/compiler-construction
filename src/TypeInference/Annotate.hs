module TypeInference.Annotate (annotateProgram, annotateFunDecl) where
...
annotateProgram s (T.Program dataDecls varDecls funDecls) =
  T.Program dataDecls (annotateVarDecl s <$> varDecls) (annotateFunMutDecl s <$> funDecls)
